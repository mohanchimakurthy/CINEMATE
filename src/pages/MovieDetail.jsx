...
export const MovieDetail = () => {
  ...
  useEffect(() => {
    async function fetchMovie() {
      const response = await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=142a24f3d242125232f2242350915350`);
    ...
    <main>
      <section className="flex justify-around flex-wrap py-5">
        ...
        <div className="max-w-2xl text-gray-700 text-lg dark:text-white">
          ...
          <div className="my-5">
          {movie.spoken_languages?.map((spoken_language,index) => (
                <span className="mr-2 border border-gray-200 rounded dark:border-gray-600 p-2" key={index}>{spoken_language.english_name}</span>
              ))}
          </div>
          <p className="my-4">
